...
<div class="content-wrapper" style="background-color: rgb(101, 125, 150); min-height: 1033px;">
	...
	<nav class="navbar navbar-expand navbar-white navbar-light" style="background-color: #404b57;">
		<div class="dropdown">
			<input type="text" class="form-control" id="ptSearch" placeholder="환자 검색" onkeyup="searchPatient(this);" style="width: 300px;">
			<ul id="ptSearchDropdown" class="dropdown-menu"></ul>
		</div>
		<div for="calendar_view">
			<button type="button" class="btn btn-outline-light ml-2 docSearch" style="display: inline-block;">
				<i class="fa-solid fa-stethoscope"></i>&nbsp;의사선택
			</button>
		</div>
		<div class="selectCss" style="display:none;">
			<select class="filter" id="emp_filter" multiple="multiple">
	       		<option value="카테고리1">카테고리1</option>
	   		</select>
   		</div>
   		<div for="calendar_view">
   			<button type="button" class="btn btn-outline-light ml-2 resvSearch" style="display: inline-block;">
   				<i class="fa-regular fa-calendar-days"></i>&nbsp;예약 현황
   			</button>
   		</div>
   		<div class="selectCss2" style="display:none;">
	   		<select class="filter resvSearch" id="type_filter" multiple="multiple">
	            <option value="카테고리1">카테고리1</option>
	        </select>
        </div>
...
$(".docSearch").on("click", function(){
	var select = $(".selectCss").css("display");
	if(select == "none"){
		$(".selectCss").show(1000);
	}else{
		$(".selectCss").hide(1000);
...
$(".resvSearch").on("click", function(){
	var select = $(".selectCss2").css("display");
	if(select == "none"){
		$(".selectCss2").show(1000);
	}else{
		$(".selectCss2").hide(1000);
	}
});
</script>